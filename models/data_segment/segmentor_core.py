import nltk
import sys
sys.path.append("/tmp/pycharm_project/Summ-N/AnyROUGE")
sys.path.append("/tmp/pycharm_project/Summ-N/utils")
from rouge import Rouge
from ThirdParty.ROUGE.rouge.rouge.rouge_score import *
#from ThirdParty.rouge.rouge.rouge_score import *
from tools import download_nltk
...
class SourceSplitterCore(object):
    ...
    def segment_one_sample_stride(self, sou):
        samples = []
        cur = []
        count = 0
        for i in range(len(sou)):
            ...
            if count + trans_len > self.k:
                if count != 0:
                    samples.append(cur)
                tmp = cur[int(len(cur)/2):]
                tmp.append(trans)
                count = len(" ".join(tmp).split())
                cur = tmp
                #cur = [trans]
            else:
                cur.append(trans)
...
class TargetSplitterCore(object):
    ...
    def _get_rouge_from_ngram(self, reference_ngrams, evaluated_ngrams)-> dict:
